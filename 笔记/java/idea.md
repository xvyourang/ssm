...
第一步:cmd打开命令行窗口，执行命令：netstat -aon|findstr “XXXX”查找占用端口的进程。（xxx为具体端口号）

第二步：根据pid，继续执行命令：taskkill -f -pid xxxx（进程号）关闭该进程。
...
5.关闭代码重复提醒， Duplicated Code

6.关闭注释警告 javadoc

7.关闭ctlrl+alt+F6  关闭0%classes
